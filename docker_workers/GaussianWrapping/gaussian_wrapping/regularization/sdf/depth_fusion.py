...
from utils.geometry_utils import is_in_view_frustum, depth_to_normal
...
def evaluate_mesh_colors_all_vertices(
    ...
    vertex_colors[~color_mask] = evaluate_mesh_colors(
        ...
        override_points=mesh.verts[~color_mask] if override_points is None else override_points[~color_mask],
    )
    return vertex_colors


def get_depth_fused_tsdf(
    pivots:torch.Tensor,
    cameras:List[Camera],
    images:List[torch.Tensor],
    depths:List[torch.Tensor],
    trunc_margin_factor:float=0.025,
    ...
    Args:
        ...
        depths (List[torch.Tensor]): List of depths
        trunc_margin_factor (float): Factor to truncate the TSDF

    Returns:
        Tuple[torch.Tensor, torch.Tensor]: SDF values and colors for the pivots
    """
    ...
    for i in range(len(cameras)):
        camera_i = cameras[i]
        rgb_i = images[i].to(pivots.device)
        depth_i = depths[i].to(pivots.device)
...
def extract_mesh(
    ...
    if pivots_colors is not None:
        verts_idx = verts_idx_list[0]  # (N_verts, 2)
    ...
    if sdf_sh is not None:
        ...
        sh_deg = int(np.sqrt(sdf_sh.shape[-1])) - 1
        ...
        sdf_factors = torch.exp(
            eval_sh(
                deg=sh_deg,
                sh=sdf_harmonics[verts_idx_list[0]].unsqueeze(-2),  # (N_verts, 2, 1, N_sh)
                dirs=edge_dir_normalized,  # (N_verts, 2, 3)
            )  # (N_verts, 2, 1)
        )  # (N_verts, 2, 1)
        
        # Linear interpolation along edges, adjusted with SDF factors
        norm_sdf = end_sdf * sdf_factors  # (N_verts, 2, 1)
        norm_sdf = norm_sdf.abs() / norm_sdf.abs().sum(dim=1, keepdim=True)  # (N_verts, 2, 1)
    else:
    ...
    if return_details:
        ...
        return Meshes(verts=verts, faces=faces, verts_colors=verts_colors), details
    else:
        return Meshes(verts=verts, faces=faces, verts_colors=verts_colors)
...
def get_pivots_occupancy(pivots_base_occupancy_logit, pivots_occupancy_shift):
    return torch.sigmoid(pivots_base_occupancy_logit + pivots_occupancy_shift)


def get_pivots_sdf(pivots_base_occupancy_logit, pivots_occupancy_shift):
    return convert_occupancy_to_sdf(get_pivots_occupancy(pivots_base_occupancy_logit, pivots_occupancy_shift))
...
# FIXME: Add refinement with fixed intersected edges
# FIXME: Add spherical harmonics
def fused_depth_with_mesh_feedback(
    pivots:torch.Tensor,
    ...
    depth_weight=0.05,
    normal_weight=0.05,
    delaunay_method:str="tetranerf",
    skip_refinement:bool=False,
    return_details:bool=False,
    mesh_normals_to_use:str="depth",  # "depth" or "normal"
):
    scene_radius = get_cameras_spatial_extent(cameras)['radius']
    device = pivots.device
    
    # Compute the Delaunay triangulation if not provided
    if tets is None:
        tets = return_delaunay_tets(pivots, method=delaunay_method)
        
    # Compute the initial SDF values by relying on adaptive TSDF fusion
    initial_pivots_sdf, initial_pivots_colors = get_depth_fused_tsdf(
        pivots=pivots,
        cameras=cameras,
        images=images,
        depths=depths,
    ...
    pivots_base_occupancy = convert_sdf_to_occupancy(initial_pivots_sdf)
    print(f"         > Initial pivots occupancy min/max: {pivots_base_occupancy.min()}, {pivots_base_occupancy.max()}")
    pivots_base_occupancy_logit = inverse_sigmoid(pivots_base_occupancy)
    print(f"         > Initial pivots occupancy logit min/max: {pivots_base_occupancy_logit.min()}, {pivots_base_occupancy_logit.max()}")
    
    _pivots_occupancy_shift = torch.nn.Parameter(
        torch.zeros_like(pivots_base_occupancy_logit, requires_grad=True).to(device)
    ).to(device)
    
    l = [{'params': [_pivots_occupancy_shift], 'lr': lr, "name": "pivots_occupancy_shift"}]
    ...
    # FIXME: Add raster_znear_factor to the arguments
    # znear = scene_radius * args.raster_znear_factor
    
    # Prepare optimization
    ema_mesh_depth_loss_for_log = 0.
    ema_mesh_normal_loss_for_log = 0.
    viewpoint_idx_stack = None
    progress_bar = tqdm(range(n_iter + 1), desc="Extraction progress")
    ...
    for i_iter in range(n_iter):
        ...
        # Compute SDF values
        pivots_sdf = get_pivots_sdf(
            pivots_base_occupancy_logit=pivots_base_occupancy_logit, 
            pivots_occupancy_shift=_pivots_occupancy_shift,
        )
        ...
        res = extract_mesh(
            ...
            pivots_colors=None,
            pivots_scale=pivots_scale,
            filter_large_edges=filter_large_edges,
            collapse_large_edges=collapse_large_edges,
            return_details=return_details and (i_iter == n_iter - 1),
        )
        if return_details and (i_iter == n_iter - 1):
            mesh, details = res
        else:
            mesh = res
        
        if skip_refinement:
            # Compute vertex colors with TSDF fusion
            _, mesh.verts_colors = get_depth_fused_tsdf(
                pivots=mesh.verts.detach(),
                cameras=cameras,
                images=images,
                depths=depths,
                trunc_margin_factor=trunc_margin_factor,
        ...
        mesh_depth = rendered_image['depth'].squeeze()
        if mesh_normals_to_use == "normal":
            mesh_normal_view = rendered_image['normals'].squeeze() @ cameras[viewpoint_idx].world_view_transform[:3, :3]
        elif mesh_normals_to_use == "depth":
            mesh_normal_view = depth_to_normal(
                cameras[viewpoint_idx],
                mesh_depth[None, :, :],
                None,
            ).permute(1, 2, 0) # (H, W, 3)
        else:
            raise ValueError(f"Invalid mesh normals to use: {mesh_normals_to_use}")
        rasterization_mask = mesh_depth > 0.
                
        # Mesh Depth Loss
        if use_depth_loss:
            supervision_depth = depths[viewpoint_idx].squeeze().to(device)
            mesh_depth_loss = torch.log(1. + (mesh_depth - supervision_depth).abs() / scene_radius)  # (H, W)
            mesh_depth_loss = depth_weight * (mesh_depth_loss * rasterization_mask).mean()
        else:
            mesh_depth_loss = torch.zeros(size=(), device=mesh.verts.device)

        # Mesh Normal Loss
        if use_normal_loss:
            # Compute normals from Gaussian depth map
            supervision_normal_view = depth_to_normal(
                cameras[viewpoint_idx],
                supervision_depth[None, :, :],
            ).permute(1, 2, 0) # (H, W, 3)

            normal_dot_product = (mesh_normal_view * supervision_normal_view).sum(dim=-1, keepdim=True)  # (H, W, 1)
            mesh_normal_loss = 1. - normal_dot_product.abs()  # (H, W, 1)
            mesh_normal_loss = normal_weight * (mesh_normal_loss * rasterization_mask.unsqueeze(-1)).mean()
            
        else:
            mesh_normal_loss = torch.zeros(size=(), device=mesh.verts.device)
        ...
        if i_iter % 100 == 0:
            ...
            # show_images(
            #     images=[
            #         predictions['images'][viewpoint_idx].permute(1, 2, 0), supervision_depth, (1. - supervision_normal_view) / 2.,
            #         rasterization_mask, mesh_depth.detach(), (1. - mesh_normal_view.detach()) / 2.,
            #     ],
            #     titles=["Image", "Depth", "Normal", "Rasterization Mask", "Mesh Depth", "Mesh Normal"],
            #     ncols=3,
            #     figsize=20,
            #     save_path=os.path.join(args.output_dir, f"refine_iter{i_iter}.png"),
            # )
    ...
    print(f"         > Number of generated vertices : {mesh.verts.shape[0]}")
    print(f"         > Number of generated faces    : {mesh.faces.shape[0]}")
