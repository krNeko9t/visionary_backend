	...
	const bool debug)
{
	...
	if(P != 0 && PN != 0)
	{
		...
		rendered = CudaRasterizer::Rasterizer::computeTransmittance(
			geomFunc, binningFunc, imgFunc, pointFunc, point_binningFunc,
			PN, P, 0, 0, // D and M are 0 as we don't need color
			W, H, settings,
			points3D.contiguous().data<float>(),
			means3D.contiguous().data<float>(),
			opacity.contiguous().data<float>(), 
	...
	return std::make_tuple(rendered, out_transmittance, radii, geomBuffer, binningBuffer);
}
