...
from ..config.loader import load_gw_train_job_config
...
from .inputs import missing_gw_train_inputs
...
def run(settings: Settings, paths: JobPaths) -> WorkerResult:
    stage_id = "gw-train"
    missing = missing_gw_train_inputs(paths, settings)
    if missing:
    ...
    config = load_gw_train_job_config(settings, paths)

    append_progress_event(
        paths.stage_events_file(stage_id),
        make_progress_event(stage_id, event_type="started", message="GW 训练开始"),
    )
    ...
    logs = run_docker_worker(
        ...
        command=config.to_train_command(CONTAINER_COLMAP_PATH, CONTAINER_OUTPUT_PATH),
